...
df <- read.csv("vitaminD_vs_aveBV_for_GAMM.csv",header = F)
...
pt_gam <- gam(V3 ~ s(V2,k=4), data = df_pt)
...
write.csv(ft$fv, "VD_fitted_GAMM_curve_for_ft.csv", row.names = FALSE)
write.csv(pt$fv, "VD_fitted_GAMM_curve_for_pt.csv", row.names = FALSE)
