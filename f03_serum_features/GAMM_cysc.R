...
setwd("/Users/szx/Desktop/paper2_preparation/paper_code_publish/03_serum_features/sp06_cysc")
df <- read.csv("cysc_vs_aveBV_for_GAMM.csv",header = F)
df <- subset(df, V2<2.5)
...
write.csv(ft$fv, "cysc_fitted_GAMM_curve_for_ft.csv", row.names = FALSE)
write.csv(pt$fv, "cysc_fitted_GAMM_curve_for_pt.csv", row.names = FALSE)
