...
df <- read.csv("albumin_vs_aveBV_for_GAMM.csv",header = F)


df_pt <- df %>% filter(V1 == 0)  # Preterm data
df_ft <- df %>% filter(V1 == 1)  # Full-term data

ft_gam <- gam(V3 ~ s(V2,k=5), data = df_ft)
pt_gam <- gam(V3 ~ s(V2,k=5), data = df_pt)
...
write.csv(ft$fv, "albumin_fitted_GAMM_curve_for_ft.csv", row.names = FALSE)
write.csv(pt$fv, "albumin_fitted_GAMM_curve_for_pt.csv", row.names = FALSE)
