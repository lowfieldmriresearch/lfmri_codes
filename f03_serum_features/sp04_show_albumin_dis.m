...
func_multi_bplot(lab_res(pt_subject_index),mri_res(pt_subject_index,mri_ind),linspace(vmin,vmax,10)+boxplotwidth,1.5,c_deeppink*0.75+0.25,boxplotwidth)
func_multi_bplot(lab_res(ft_subject_index),mri_res(ft_subject_index,mri_ind),linspace(vmin,vmax,10),1.5,c_deepblue*0.75+0.25,boxplotwidth)
...
albumin_res(:,exc_ind) = [];
writematrix(albumin_res','albumin_vs_aveBV_for_GAMM.csv');

%%
% % layer #2, GAMM fitted curve and CI,
gamm_res = readmatrix('albumin_fitted_GAMM_curve_for_ft.csv');
gamm_res(:,4) =gamm_res(:,4);
gamm_res(:,5) =gamm_res(:,5);
fill([gamm_res(:,1)', fliplr(gamm_res(:,1)')], [gamm_res(:,4)', fliplr(gamm_res(:,5)')], c_deepblue*0.75, 'FaceAlpha',0.35 ); hold on;
...
gamm_res = readmatrix('albumin_fitted_GAMM_curve_for_Pt.csv');
gamm_res(:,4) =gamm_res(:,4);
gamm_res(:,5) =gamm_res(:,5);
