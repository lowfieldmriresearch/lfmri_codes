...
func_multi_bplot(lab_res(pt_subject_index),mri_res(pt_subject_index,mri_ind),linspace(vmin,vmax,10)+thewidth,3,c_deeppink*0.75+0.25,thewidth)
func_multi_bplot(lab_res(ft_subject_index),mri_res(ft_subject_index,mri_ind),linspace(vmin,vmax,10),3,c_deepblue*0.75+0.25,thewidth)
...
albumin_res(:,exc_ind) = [];
writematrix(albumin_res','creatinine_vs_aveBV_for_GAMM.csv');
%%
% % layer #2, GAMM fitted curve and CI,
gamm_res = readmatrix('creatinine_fitted_GAMM_curve_for_ft.csv');
...
gamm_res = readmatrix('creatinine_fitted_GAMM_curve_for_Pt.csv');
gamm_res(:,4) =gamm_res(:,4);
gamm_res(:,5) =gamm_res(:,5);
fill([gamm_res(:,1)', fliplr(gamm_res(:,1)')], [gamm_res(:,4)', fliplr(gamm_res(:,5)')], c_deeppink*0.75, 'FaceAlpha',0.35);
plot(gamm_res(:,1),gamm_res(:,2),'color',c_deeppink*0.75,'linewidth',2);
plot(gamm_res(:,1),gamm_res(:,4),'color',c_deeppink*0.75,'linewidth',2,'linestyle','--');
plot(gamm_res(:,1),gamm_res(:,5),'color',c_deeppink*0.75,'linewidth',2,'linestyle','--');
...
xlabel('Creatinine (umol/L)')
ylim([-4,4]);

set(findall(gcf,'-property','fontweight'),'fontweight','bold');
set(findall(gcf,'-property','fontsize'),'fontsize',14);
title('Creatinine','fontsize',16)
